...
class CreateCar(BaseCommand):
    ...
    def create_car(self, path, template='dell2', overwrite=False):
        """
        This script sets up the folder struction for dell to work.
        It must run without dell installed so that people installing with
        docker can build the folder structure for docker to mount to.
        """

        #these are neeeded incase None is passed as path
        path = path or '~/mycar'
        template = template or 'dell2'
...
class CalibrateCar(BaseCommand):
    ...
    def run(self, args):
        from parts.actuator import PCA9685
...
class Sim(BaseCommand):
    """
    Start a websocket SocketIO server to talk to a dell simulator
    """
    ...
    def run(self, args):
        ...
        """
        import socketio
        from parts.simulation import SteeringServer
        from parts import KerasCategorical, KerasLinear
...
class ShowPredictionPlots(BaseCommand):
    ...
    def plot_predictions(self, cfg, tub_paths, model_path):
        ...
        """
        from parts import TubGroup
        from parts import KerasCategorical
...
def execute_from_command_line():
    """
    This is the fuction linked to the "dell" terminal command.
    """
    commands = {
            'createcar': CreateCar,
            'findcar': FindCar,
    ...
    if command_text in commands.keys():
        ...
        util.proc.eprint('Usage: The availible commands are:')
        util.proc.eprint(list(commands.keys()))

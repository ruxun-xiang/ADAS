...
class KerasCategorical(KerasPilot):
    ...
    # def run(self, img_arr_a, img_arr_b):
        #start_time = time.time()
        try:
            img_arr = img_arr.reshape((1,) + img_arr.shape)
            angle_binned, throttle = self.model.predict(img_arr)
            angle_unbinned = dellcar.util.data.linear_unbin(angle_binned[0])
...
def default_categorical():
    img_in = Input(shape=(240, 320, 3), #(240, 640, 3),
    ...
    throttle_out = Dense(15, activation='relu', name='throttle_out')(x)  # Reduce to 1 number, Positive number only

...
class Tub(object):
    ...
    >>> path = '~/mydell/test_tub'
    >>> inputs = ['user/speed', 'cam/image_a', 'cam/image_b']
    >>> types = ['float', 'image', 'image']
    ...
    def put_record(self, data):
        ...
        for key, val in data.items():
            ...
            if typ in ['str', 'float', 'int', 'boolean']:
                ...
                # if key == 'cam_rgb/image_array':
                name = self.make_file_name(key, ext='.jpg')
                # else:
                #   name = self.make_file_name(key, ext='.jpg', depth = True)
...
class TubGroup(Tub):
    def __init__(self, tub_paths_arg):
        tub_paths = dellcar.util.files.expand_path_arg(tub_paths_arg)
